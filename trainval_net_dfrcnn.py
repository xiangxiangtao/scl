    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            if args.dataset == 'sim10k':
                loss += (dloss_s.mean() + dloss_t.mean() + dloss_s_p.mean() + dloss_t_p.mean() + cst_loss_s.mean() + cst_loss_t.mean()) * args.eta
            else:
                loss += (dloss_s.mean() + dloss_t.mean() + dloss_s_p.mean() + dloss_t_p.mean() + cst_loss_s.mean() + cst_loss_t.mean())
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if step % args.disp_interval == 0:
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f dloss s: %.4f dloss t: %.4f dloss s pixel: %.4f dloss t pixel: %.4f consistency_s: %.4f consistency_t: %.4f eta: %.4f" \
                    % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, dloss_s, dloss_t, dloss_s_p, dloss_t_p,cst_loss_s, cst_loss_t,
                       args.eta))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
